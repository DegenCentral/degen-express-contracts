...
import { ISolidlyLauncher } from "../../interfaces/ISolidlyLauncher.sol";
...
contract DegenTokens is Ownable {
	...
	function launch(address token, TokenType t) external onlyOwner payable {
		...
		Token(token).transfer(degen.launcher, tokens);
		(address pair, uint256 lp) = ISolidlyLauncher(degen.launcher).launch{ value: eth + msg.value }(token, tokens, msg.value);

		address vault = Locker(address(this))._lock(pair, lp, creator);
